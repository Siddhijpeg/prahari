...
import { interventionValue, formatCompactINR } from '../utils/interventionValue';
...
// Model/version string stamped onto every approval — same idea as a commit hash,
// so an auditor can always tell exactly which model produced the number they're approving.
const MODEL_VERSION = 'risk-model v0.4.2 · registry v1.1';

export default function AlertCenter({ onOpenCase }: { onOpenCase?: () => void }) {
  const [activeTab, setActiveTab] = useState('all');
  const [acknowledgedAlerts, setAcknowledgedAlerts] = useState<Record<string, boolean>>({});

  // Approval gate: alerts whose timeline already shows "Bank notified" are treated as
  // already-approved (backdated, for demo continuity). Anything earlier than that is
  // genuinely pending — a supervisor has to approve before the bank/LEA buttons unlock.
  const [approvals, setApprovals] = useState<Record<string, { approved: boolean; by?: string; at?: string }>>(
    () => {
      const initial: Record<string, { approved: boolean; by?: string; at?: string }> = {};
      alerts.forEach(a => {
        initial[a.id] = a.timeline.includes('Bank notified')
          ? { approved: true, by: 'K. Verma (Supervisor)', at: 'earlier today' }
          : { approved: false };
      });
      return initial;
    }
  ...
  function approveAlert(id: string) {
    ...
    setApprovals(prev => ({ ...prev, [id]: { approved: true, by: 'K. Verma (Supervisor)', at: now } }));
  }

  const totalRecoverable = alerts.reduce(
    (sum, a) => sum + interventionValue({ confidence: a.risk, amount: a.amount, window: a.window }),
    0
  );
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#7C5CFC]/8 border border-[#7C5CFC]/25 text-[#7C5CFC] text-xs font-semibold">
            <span className="font-mono">{formatCompactINR(totalRecoverable)}</span> recoverable right now
          </div>
            ...
            <div className="p-5">
              <div className="grid grid-cols-12 gap-4">
                ...
                <div className="col-span-8">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        {[
                          { label: 'Expected Window', value: alert.window, highlight: false },
                          { label: 'Amount at Risk', value: alert.amount, highlight: false },
                          {
                            label: 'Est. Recoverable',
                            value: formatCompactINR(interventionValue({ confidence: alert.risk, amount: alert.amount, window: alert.window })),
                            highlight: true,
                          },
                          { label: 'Relevant Bank', value: alert.bank, highlight: false },
                        ].map(d => (
                          <div
                            key={d.label}
                            className={`rounded-xl p-2.5 border ${d.highlight ? 'bg-[#7C5CFC]/6 border-[#7C5CFC]/25' : 'bg-[#F7F8FA] border-[#E2E8F0]'}`}
                          >
                            <div className={`text-[10px] font-medium ${d.highlight ? 'text-[#7C5CFC]' : 'text-[#94A3B8]'}`}>{d.label}</div>
                            <div className={`text-sm font-bold mt-0.5 font-mono ${d.highlight ? 'text-[#7C5CFC]' : 'text-[#0F172A]'}`}>{d.value}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Approval Gate */}
                  {approvals[alert.id]?.approved ? (
                    <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200 text-xs">
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="6" fill="#10B981"/><path d="M3.8 6.6L5.6 8.4L9.2 4.8" stroke="white" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      <span className="text-emerald-700 font-medium">
                        Approved by {approvals[alert.id].by} · {approvals[alert.id].at} · {MODEL_VERSION}
                      </span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-between gap-2 mb-3 px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-xs">
                      <span className="text-amber-700 font-medium">
                        Pending supervisor approval — bank/LEA notification is locked until then.
                      </span>
                      <button
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <button
                      disabled={!approvals[alert.id]?.approved}
                      className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white rounded-xl transition-colors ${
                        approvals[alert.id]?.approved ? 'bg-[#E5484D] hover:bg-[#D43840]' : 'bg-[#E5484D]/30 cursor-not-allowed'
                      }`}
                    >
                    ...
                    <button
                      disabled={!approvals[alert.id]?.approved}
                      className={`flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white rounded-xl transition-colors ${
                        approvals[alert.id]?.approved ? 'bg-[#7C5CFC] hover:bg-[#6D4FF5]' : 'bg-[#7C5CFC]/30 cursor-not-allowed'
                      }`}
                    >
